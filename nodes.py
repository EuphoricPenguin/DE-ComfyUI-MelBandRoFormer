...
# Use the diffusion_models directory for config files
folder_paths.add_model_folder_path("melband_configs", os.path.join(folder_paths.models_dir, "diffusion_models"))
...
class MelBandRoFormerModelLoader:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                "config_name": ([""] + folder_paths.get_filename_list("melband_configs"), {"tooltip": "Optional YAML config file from 'ComfyUI/models/diffusion_models' folder. If empty, uses default config."}),
            },
        }
    ...
    def loadmodel(self, model_name, config_name=""):
        ...
        if config_name:
            try:
                config_path = folder_paths.get_full_path_or_raise("melband_configs", config_name)
                with open(config_path, 'r', encoding='utf-8') as f:
                    # Use full loader to handle Python-specific tags like !!python/tuple
                    yaml_config = yaml.load(f, Loader=yaml.FullLoader)
                
                # Extract model configuration from YAML
                if 'model' in yaml_config:
                    model_config.update(yaml_config['model'])
                
                print(f"Loaded model configuration from: {config_name}")
                
            except Exception as e:
                print(f"Error loading YAML config file '{config_name}': {e}")
                print("Using default configuration instead.")

        # Convert tuple from YAML to tuple for Python
        if 'multi_stft_resolutions_window_sizes' in model_config:
            if isinstance(model_config['multi_stft_resolutions_window_sizes'], list):
